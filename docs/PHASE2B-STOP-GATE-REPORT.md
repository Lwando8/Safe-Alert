...
Migrated incident/push callables and `/ops/incidents` enforce server-resolved `organizationId` with active membership + permissions. Firebase emulator isolation probes pass. Clerk live authentication and physical-device paths are not verified in this environment.

## 2. Code changes (high level)

| Area | Change |
|---|---|
| `tenantIncidentService` | Shared create/list/push helpers |
| Functions `index.ts` | Uses helpers; adds `listOrgIncidents` |
| Membership sync + webhook | Idempotency, org.updated, fail-closed, suspend preserve |
| `/ops/incidents` + `/api/ops/incidents` | Real tenant-scoped backend wiring |
| Tests + seed/probe scripts | Automated isolation evidence |
| Docs | Implementation notes, checklist, evidence, inventory, bridge, this report |
...
`/ops/incidents` uses the real backend (`loadOpsIncidentsForSession` → Firestore filtered by membership org). States implemented: loading, empty, error, unauthorized, unavailable, clerk-unconfigured. Live Clerk UI probe externally blocked.

## 5. Verification evidence

- Tests: **33/33 passed**
- Emulator probes: **8/8 passed**
- Clerk probes: **externally blocked**
- See `PHASE2B-TEST-EVIDENCE.md`
