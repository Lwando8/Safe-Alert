...
exports.acceptIncident = (0, https_1.onCall)(async (req) => {
    ...
    try {
        const { buildAcceptIncidentAccessGrant } = await Promise.resolve().then(() => __importStar(require('./services/accessGrants')));
        ...
        const { recordAuditEvent } = await Promise.resolve().then(() => __importStar(require('./audit/recordAuditEvent')));
        await recordAuditEvent({
            organizationId: context.organizationId,
            siteId: data.siteId || context.siteId || null,
...
exports.createRideSafetyRequestCallable = (0, https_1.onCall)(async (req) => {
    const context = await (0, requestContext_1.resolveRequestContextFromCallable)(req);
    const { createRideSafetyRequest } = await Promise.resolve().then(() => __importStar(require('./services/rideSafetyService')));
    return createRideSafetyRequest(context, req.data || {});
});
exports.listRideSafetyRequestsCallable = (0, https_1.onCall)(async (req) => {
    const context = await (0, requestContext_1.resolveRequestContextFromCallable)(req);
    const { listRideSafetyRequests } = await Promise.resolve().then(() => __importStar(require('./services/rideSafetyService')));
 ...
 */
exports.issueFirebaseBridgeTokenCallable = (0, https_1.onCall)(async (req) => {
    let context = null;
    try {
        context = await (0, requestContext_1.resolveRequestContextFromCallable)(req);
    }
    ...
    const { issueFirebaseBridgeToken } = await Promise.resolve().then(() => __importStar(require('./services/firebaseBridge')));
    return issueFirebaseBridgeToken({
        context,
        firebaseUidFromAuth,
        operatorSecret: typeof req.data?.operatorSecret === 'string' ? req.data.operatorSecret : undefined,
        targetFirebaseUid: typeof req.data?.firebaseUid === 'string' ? req.data.firebaseUid : undefined,
    });
});
